...
export default function MqttSubscriber({ deviceId }) {
  ...
  const exportCSV = () => {
    ...
    const rows = temperatureData.map((data, index) => {
      ...
      if (selectedFields.humidity)
        row.push(humidityData[index]?.humidity || "");
      if (selectedFields.moisture)
        row.push(moistureData[index]?.moisture || "");
      return row;
    });
  ...
  return (
    <div className="pl-20 pr-20 pb-20 shadow-lg bg-white text-center">
      <div className="flex items-center justify-center">
        ...
        {isLoading && (
          <FaSpinner
            className="animate-spin text-lg text-gray-500 ml-4"
